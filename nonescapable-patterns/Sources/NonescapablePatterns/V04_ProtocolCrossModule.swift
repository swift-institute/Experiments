...
// Date: 2026-04-01
//
// Origin: escapable-protocol-cross-module

import PathPrimitivesLib
...
enum V04_ProtocolCrossModule {
    ...
    static func decompose<V: Path.`Protocol` & ~Copyable & ~Escapable>(
        _ view: borrowing V
    ) where V.Char == UInt8 {
        if let parent = view.parentBytes {
