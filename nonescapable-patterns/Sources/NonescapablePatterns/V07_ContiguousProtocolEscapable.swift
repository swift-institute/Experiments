...
// Date: 2026-03-19
//
// Origin: contiguous-protocol-escapable

enum V07_ContiguousProtocolEscapable {

    // ============================================================
    // The protocol — property with @lifetime
    // COMPILE ERROR (expected): '@lifetime' attribute cannot be applied to this declaration
    // ============================================================

    // COMPILE ERROR (expected): @lifetime on protocol property not implemented
    ...
    // ============================================================
    // Workaround protocol — no @lifetime on property, use method instead
    // ============================================================
    ...
    struct OwnedBuffer: ~Copyable, ContiguousProtocol {
        ...
        @_lifetime(borrow self)
        borrowing func span() -> Span<UInt8> {
    ...
    struct CopyableBuffer: ContiguousProtocol {
        let bytes: [UInt8]
        @_lifetime(borrow self)
        borrowing func span() -> Span<UInt8> { bytes.span }
    }
    ...
    static func genericSum<C: ContiguousProtocol & ~Copyable & ~Escapable>(
    ...
    static func makeString<C: ContiguousProtocol & ~Copyable & ~Escapable>(
        _ source: borrowing C
    ) -> String where C.Element == UInt8 {
        String(copying: unsafe UTF8Span(unchecked: source.span()))
    }

    static func spansEqual<A: ContiguousProtocol & ~Copyable & ~Escapable,
    ...
    static func run() {
        print("=== ContiguousProtocol ~Escapable + method workaround ===\n")
        ...
        print("\nNote: @lifetime on protocol PROPERTY remains BLOCKED.")
        print("Workaround: use @_lifetime on protocol METHOD instead.")
        print("\n=== V07 All passed ===")
    }
}
