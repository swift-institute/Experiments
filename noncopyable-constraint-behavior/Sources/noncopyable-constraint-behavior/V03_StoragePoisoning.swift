...
// Origin: noncopyable-storage-poisoning

enum V03_StoragePoisoning {
    ...
    // COMPILE ERROR (expected): Poisoning conformance — adding Sequence
    // causes Storage's UnsafeMutableBufferPointer<Element> stored property
    // to require Element: Copyable, even with the `where Element: Copyable` guard.
    //
    ...
    static func run() {
        let s = Storage<NonCopyableValue>(capacity: 8)
