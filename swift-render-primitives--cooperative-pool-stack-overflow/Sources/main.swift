// Experiment: cooperative-pool-stack-overflow
// Date: 2026-03-18
// Toolchain: Swift 6.2
// Status: CONFIRMED
//
// Reduced reproduction of SIGBUS on Swift's cooperative thread pool (~544 KB
// stack) caused by recursive _render → body → _render chains in a
// result-builder-based rendering system.
//
// Each nesting level adds ~2.8 KB of stack (9 frames: View._render,
// body.getter, 6× Styled._render, Tag._render). At depth 180 the
// cumulative usage exceeds the cooperative pool budget → SIGBUS.
//
// The same view tree renders without issue on the main thread (8 MB stack).
//
...
// MARK: - Minimal rendering infrastructure (mirrors Render Primitives Core)
...
// MARK: - Simulated HTML element and CSS modifier
...
// MARK: - Leaf: 10 cells × 6 CSS modifiers each
...
// MARK: - 200 nesting levels
//
// Each level wraps the previous in Tag + 6× Styled, producing 9 stack frames
// per level (~2.8 KB). Cooperative pool overflows at ~180 levels.
//
// In production, only ~20 levels suffice because PDF.HTML.Context operations
// add ~11 KB per frame. The mechanism is identical.
...
N200._render(N200())
print("OK — depth 200 renders without issue")

print("Async (cooperative pool, ~544 KB): ", terminator: ""); fflush(stdout)
await Task.detached {
    N200._render(N200())
}.value
print("SIGBUS expected before this line")
