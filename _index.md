...
| member-import-visibility-body-conflict | MemberImportVisibility + stored `body` property: does `public import SwiftUI` in same file cause SwiftUI.View.Body collision? 10 variants. V1–V5 rule out import-level explanations (REFUTED); V6 establishes the rename mechanism with `Content` (CONFIRMED); V7 (`@retroactive`) and V8 (SE-0491 module selectors) are REFUTED; V9 (wrapper escape hatch) and V10 (`Render.Body` + `associatedtype Rendered`, the blog-recommended shape) are CONFIRMED. | 2026-03-13 (V10 added 2026-04-15) | Swift 6.3 | CONFIRMED |
...
| silgen-thunk-noncopyable-sending-capture | SILGen reabstraction-thunk SIGSEGV (signal 11) when a `~Copyable & Sendable` value is moved into an Optional slot, captured by a `sending () throws -> sending Value` thunk, and the thunk is passed into an `@Sendable` async closure. Adjacent runtime variant: SIGABRT "freed pointer was not the last allocation" on first suspension. Workaround: two-callback storage (denotationally equivalent tagged union). Surfaced during `swift-effect-primitives` `~Copyable` widening. | 2026-04-17 | Swift 6.3.1 | CONFIRMED |
...
| ~~predictable-memopt-destroy-value-source~~ | Root-cause investigation of #88022 (spurious `destroy_value` in CopyPropagation). Documentation-only, deleted — #88022 fixed in Swift 6.3. | 2026-03-19 | Swift 6.2.4 | SUPERSEDED & DELETED |
...
| docc-merge-multi-archive-hosting | `docc merge` combining three article-only DocC archives into a single hosted site for swift-institute.org Research/Experiments integration. Unified detailedGrid landing + cross-archive sidebar nav CONFIRMED on Swift 6.3.1 / macOS 26. Full-text search REFUTED (pre-existing DocC constraint, not merge-specific). See Research/docc-search-capabilities-and-merged-site-strategy.md. | 2026-04-17 | Swift 6.3.1 | PARTIAL (a+b CONFIRMED, c REFUTED) |
